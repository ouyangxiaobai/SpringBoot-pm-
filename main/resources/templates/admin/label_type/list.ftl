...
<title>${siteName!""}|标的物类型管理-${title!""}</title>
                ...
                <div class="table-responsive">
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      ...
                      <#list pageBean.content as labelType>
                      <tr>
                        <td style="vertical-align:middle;">
                          <label class="lyear-checkbox checkbox-primary">
                            <input type="checkbox" name="ids[]" value="${labelType.id}"><span></span>
                          </label>
                        </td>
                        <td style="vertical-align:middle;">${labelType.name}</td>
                        <td style="vertical-align:middle;">
                        	<#if labelType.status == 1>
                        	<font class="text-success">正常</font>
                        	<#else>
                        	<font class="text-warning">冻结</font>
                        	</#if>
                        </td>
                        <td style="vertical-align:middle;" style="vertical-align:middle;"><font class="text-success">${labelType.createTime}</font></td>
                      </tr>
                    </#list>
                    <#else>
...
function deleteReq(id,url){
	$.ajax({
		...
		success:function(data){
			if(data.code == 0){
				showSuccessMsg('标的物类型删除成功!',function(){
