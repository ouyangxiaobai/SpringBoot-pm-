                ...
                <form id="search-form-projectSign">
                    <div class="per_edit_search">
                        <div class="per_edit_list">
                            ...
                            <select name='status' id="projectSignStatus">
                ...
                <div class="per_table">
                    <table border="1" borderColor="#ECE3E3">
                        ...
                        <#if pageBean.content?size gt 0>
                            <#list pageBean.content as biddingRecord>
                                <tr>
                                    <td style="vertical-align:middle;">${biddingRecord.biddingProject.projectNumber}</td>
                                    <td style="vertical-align:middle;"><a href="/home/bidding/detail?id=${biddingRecord.biddingProject.id}">
                                            <#if biddingRecord.biddingProject.title?length gt 4>
                                                ${biddingRecord.biddingProject.title?substring(0,4)}...
                                            <#else>
                                                ${biddingRecord.biddingProject.title}
                                            </#if>
                                        </a></td>
                                    <td style="vertical-align:middle;">${biddingRecord.biddingProject.startTime}</td>
                                    <td style="vertical-align:middle;">${biddingRecord.biddingProject.biddingStartTime}</td>
                                    <td style="vertical-align:middle;">${biddingRecord.bid}</td>
                                    <td style="vertical-align:middle;">
                                        <#if biddingRecord.biddingProject.projectStatus.getCode() == 3>
                                            公示中
                                        <#elseif biddingRecord.biddingProject.projectStatus.getCode() == 4>
                                            竞价中
                                        <#elseif biddingRecord.biddingProject.projectStatus.getCode() == 5>
                                            竞价成功
                                        <#elseif biddingRecord.biddingProject.projectStatus.getCode() == 6>
                                            竞价结束
                                        <#else>
                                            已成交
                                        </#if>
                                    ...
                                    <td style="vertical-align:middle;">
                                        <#if biddingRecord.biddingProject.projectStatus.getCode() == successStatus>
                                            <button class="layui-btn layui-btn-primary layui-btn-sm" type="button" onclick="payMoney(${biddingRecord.id})">
                                                支付
                                            </button>
                                        <#else>
                    ...
                    <#if pageBean.total gt 0>
                        <ul class="pagination ">
                            ...
                            <#else>
                                <li><a href="record?status=${projectTimeVO.status}&startTime=${projectTimeVO.startTime!""}&endTime=${projectTimeVO.endTime!""}&biddingStartTime=${projectTimeVO.biddingStartTime!""}&biddingEndTime=${projectTimeVO.biddingEndTime!""}&currentPage=1">«</a></li>
                            </#if>
                            <#list pageBean.currentShowPage as showPage>
                                <#if pageBean.currentPage == showPage>
                                    <li class="active"><span>${showPage}</span></li>
                                <#else>
                                    <li><a href="record?status=${projectTimeVO.status}&startTime=${projectTimeVO.startTime!""}&endTime=${projectTimeVO.endTime!""}&biddingStartTime=${projectTimeVO.biddingStartTime!""}&biddingEndTime=${projectTimeVO.biddingEndTime!""}&currentPage=${showPage}">${showPage}</a></li>
                                </#if>
                            </#list>
                            <#if pageBean.currentPage == pageBean.totalPage>
                                <li class="disabled"><span>»</span></li>
                            <#else>
                                <li><a href="record?status=${projectTimeVO.status}&startTime=${projectTimeVO.startTime!""}&endTime=${projectTimeVO.endTime!""}&biddingStartTime=${projectTimeVO.biddingStartTime!""}&biddingEndTime=${projectTimeVO.biddingEndTime!""}&currentPage=${pageBean.totalPage}">»</a></li>
                            </#if>
...
<script type="text/javascript">
    ...
    function searchBidding() {
        ...
        window.location.href="/home/bidding/record?"+data
    }

    function payMoney(biddingRecordId) {
        $.ajax({
            url:'payMoney',
            type:'post',
            data:{
